...
``llm`` row is no longer "entirely fallback" and drops out of future backfill
candidate sets. Old fallback rows for keywords the new extraction didn't
happen to repeat are not retroactively removed; that is an accepted
consequence of the locked append-only truth design (PRD §5), not a bug in this
module.
